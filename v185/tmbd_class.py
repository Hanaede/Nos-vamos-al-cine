...
class Tmdbclient:
    ...
    # Opción 4
    def get_trending_week_movies_by_genre(self):
        try:
            self.internet_check()
            genre_id = input("Introduce el ID del género de película para obtener las películas trending (pulsa intro si quieres ver por todos los géneros): ")

            if genre_id is None:
                trending_info = self.get_info_trending_week()  # Obtener trending semanal por defecto
            else:
                trending_info = self.get_info_trending_by_genre(genre_id)

            if trending_info['results']:
                trending_movies = []
                for movie in trending_info['results']:
                    trending_movies.append(movie['title'])
                self.save_to_json({"Trending": trending_movies})
    ...
    def response_trending_by_genre(self, genre_id, page):
        url = f"{self.__BASE_URL}/discover/movie"
        params = {'api_key': self.__API_KEY, 'with_genres': genre_id, 'page': page}
        response = requests.get(url, params=params)
        response.raise_for_status()
        trending_info = response.json()
